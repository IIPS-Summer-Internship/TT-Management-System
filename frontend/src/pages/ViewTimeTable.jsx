...
const ViewTimeTable = () => {
  ...
  const fetchLectures = async () => {
    ...
    try {
      ...
      // Construct query parameters based on the selected filters
      const queryParams = new URLSearchParams();

      // Prioritize filtering by Faculty or Room if selected
      if (selectedFaculty) {
          queryParams.append('facultyId', selectedFaculty.ID);
      } else if (selectedRoom) {
          queryParams.append('roomId', selectedRoom.ID);
      } else if (selectedBatch && semesterNumber !== undefined) {
           // If Faculty and Room are not selected, filter by Course, Batch, and Semester
            queryParams.append('batchId', selectedBatch.ID);
            queryParams.append('semester', semesterNumber);
            // Assuming your backend can also filter by CourseID with BatchID and Semester
             if(selectedCourse) queryParams.append('courseId', selectedCourse.ID);

       } else {
           // No valid criteria selected for fetching lectures
           setIsLoadingLectures(false);
           return;
       }


      const response = await fetch(`${API_ENDPOINTS.LECTURE}?${queryParams.toString()}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include'
      });
  ...
  // --- Button Handlers ---
   const handleGenerateTimetable = () => {
       // Fetch timetable only if course, batch, and semester are selected
       if (selectedFilters.course !== undefined && selectedFilters.batch !== undefined && selectedFilters.semester !== undefined) {
           // Ensure faculty and room filters are cleared when generating the main timetable
            setSelectedFilters(prevFilters => ({
                ...prevFilters,
                faculty: undefined,
                room: undefined
            }));
           fetchLectures(); // Fetch timetable based on course, batch, semester
       } else {
           alert("Please select Course, Batch, and Semester to generate timetable.");
       }
   };
          ...
          <div className="p-6">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
              <Button
                ...
                disabled={selectedFilters.course === undefined || selectedFilters.batch === undefined || selectedFilters.semester === undefined || loading || isLoadingLectures}
              >
                 Generate Timetable
              </Button>
               <Button
